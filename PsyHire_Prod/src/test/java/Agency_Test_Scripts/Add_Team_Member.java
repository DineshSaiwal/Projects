...
import Agency_Scenarios.Add_Team_Member_Page;
import Base_Package.Base_Class;
import Base_Package.Login;
import Base_Package.ScreenRecorderUtil;
import Base_Package.Utility_Class;

public class Add_Team_Member extends Base_Class {
ExtentTest test;
	...
	void Add_Team_Member_Validation() throws Exception{
		...
		test = extent.createTest("Add_Team_Member").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Agency");
		test.info(MarkupHelper.createLabel("Test Case Executed On { "+ browserName +" } Browser", ExtentColor.BLUE));
		ScreenRecorderUtil.startRecord("Add_TeamMember_Validation");
		...
		Login.Agency_Login();
		 ...
		 try {page.Enter_Name();
 		 ...
 		 try {page.Click_On_Add();
		 	test.pass(MarkupHelper.createLabel("Clicked on Add Button", ExtentColor.GREEN)); Thread.sleep(2000);
		 	test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); } 
		 catch (Exception e) {
...
//		 Thread.sleep(2000);
//		 try {Utility_Class.Toast_Message_Validation("Team member added Team member is added to your team in a few seconds");
