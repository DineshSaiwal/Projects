...
import Oragnization_Scenarios.Psy_Assessment_Page_Create;

public class Psy_Assessment_Create_Validation extends Base_Class{
	ExtentTest test;
	public String Messages;
		@Test 
		void Psy_Assessment_create_Validation() throws Exception{
			
			
		test = extent.createTest("Psy_Assessment_Create_Validation").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Organization");
		...
		ScreenRecorderUtil.startRecord("Psy_Assessment_Create_Validation");
		WebDriver driver = getDriver();
		Psy_Assessment_Page_Create page= new Psy_Assessment_Page_Create(driver);	
		...
		Thread.sleep(1000);
		 ...
		 try {page.Click_On_Psy_Assessments();
		 	test.pass(MarkupHelper.createLabel("Clicked on Psy_Assessments", ExtentColor.GREEN));} 
		 catch (Exception e) {
			test.fail(MarkupHelper.createLabel("Failed to Click on Psy_Assessments",  ExtentColor.RED));
			test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); 
			test.log(Status.FAIL, "Exception occurred ==> " + e.getMessage());
			ScreenRecorderUtil.stopRecord();
				}
		 ...
		 try {Utility_Class.Toast_Message_Validation("Added Question");
		 	test.pass(MarkupHelper.createLabel("Psy-Assessment Folder Created Successfully", ExtentColor.GREEN));
		 	test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot());  } 
		 catch (Exception e) {
			test.fail(MarkupHelper.createLabel("Failed to add question", ExtentColor.RED));
			test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); 
