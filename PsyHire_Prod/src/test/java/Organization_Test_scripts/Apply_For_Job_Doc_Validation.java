...
import Oragnization_Scenarios.Apply_For_Job_Doc_page;

public class Apply_For_Job_Doc_Validation extends Base_Class{
	ExtentTest test;
	
	@Test 
	void Apply_For_Job_Doc_validation() throws Exception{
		
		
		test = extent.createTest("Apply_For_Job_Doc_Validation").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Organization");
		...
		ScreenRecorderUtil.startRecord("Apply_For_Job_Doc_Validation");
		WebDriver driver = getDriver();
		Apply_For_Job_Doc_page page= new Apply_For_Job_Doc_page(driver);	
		...
		Thread.sleep(1000);
		 ...
		 try {page.Upload_Cover_Letter();
		 	test.pass(MarkupHelper.createLabel("Uploaded Cover Letter In Doc. Format", ExtentColor.GREEN));} 
		 catch (Exception e) {
		 ...
		 Thread.sleep(1000);
		 try {page.Upload_Resume_Doc();
		 	test.pass(MarkupHelper.createLabel("Uploaded Resume In Doc. Format", ExtentColor.GREEN));} 
		 catch (Exception e) {
			test.fail(MarkupHelper.createLabel("Failed to Upload Resume In Doc. Format",  ExtentColor.RED));
			test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); 
			test.log(Status.FAIL, "Exception occurred ==> " + e.getMessage());
			ScreenRecorderUtil.stopRecord();
				}
