...
import Oragnization_Scenarios.Invite_Students_Manual_page;
...
ExtentTest test;
	...
	void Invite_Students_Validation() throws Exception{
		...
		test = extent.createTest("Invite_Students_Manually").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Organization");
		 ...
		 try {page.Click_on_Jobs_Tab();
			...
			ScreenRecorderUtil.stopRecord();
			 ...
			 try {page.Click_on_Invite_Student();
			 	test.pass(MarkupHelper.createLabel("Clicked on Invite Candidate", ExtentColor.GREEN));} 
			 catch (Exception e) {
			 ...
			 Thread.sleep(1000);
			 try {Utility_Class.Toast_Message_Validation("Invited successfully");
