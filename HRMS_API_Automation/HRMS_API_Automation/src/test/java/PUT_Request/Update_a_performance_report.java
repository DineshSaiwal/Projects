...
public class Update_a_performance_report extends Login {
	static ExtentTest Message;
	//********************* UNKNOWN BODY PARAMETER*************************
	@Test
	public static void update_an_existing_Grade() throws IOException {
		Message=extent.createTest("Update_a_performance_report").assignAuthor(AutherT).assignCategory(Put);
		login();
		...
		Response response = RestAssured.given()
				...
				.put(base_url+Path.Update_a_performance_report);
...
//Validation 
		if(response.getStatusCode()==200){
			Message.pass(MarkupHelper.createLabel("Update_a_performance_report", ExtentColor.GREEN));			
			Message.info(MarkupHelper.createLabel((String) JP.get("message"), ExtentColor.BLUE));
			Message.info(MarkupHelper.createLabel(response.getStatusLine(), ExtentColor.BLUE));
			Message.info(MarkupHelper.createCodeBlock(response.getBody().asString(), CodeLanguage.JSON));}
		else{
			Message.fail(MarkupHelper.createLabel("Update_a_performance_report", ExtentColor.RED));
